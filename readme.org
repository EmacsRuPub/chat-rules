...
* Популяризация /CUSTOMIZE/ интерфейса
Часть чата *@emacs_ru* глубоко убеждена в том, что /customize/ интерфейс -
незаслуженно непопулярен.  Здесь собраны ссылки на ресурсы с объяснениями,
почему /customize/ является отличным инструментом для конфигурации *GNU/Emacs* в
21ом веке.
- [[https://www.liltechdude.xyz/articles/custom-is-based.html][Кастом это БАЗа]]
